...
const Login = ({ onLogin }) => {
    const [email, setEmail] = useState(""); const [pass, setPass] = useState(""); const [err, setErr] = useState(""); const [sel, setSel] = useState(null);
    ...
    const go = () => { const u = USERS.find(u => u.email === email && u.password === pass); if (!u) { setErr("Invalid credentials. Try a demo role below."); return; } onLogin(u); };
    return (
        <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#0a2463 0%,#1e56c2 45%,#0077b6 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, position: "relative", overflow: "hidden" }}>
            ...
            <div className="slide-up" style={{ width: "100%", maxWidth: 460, position: "relative" }}>
                <div style={{ textAlign: "center", marginBottom: 28 }}>
                    <div style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 68, height: 68, borderRadius: 20, background: "rgba(255,255,255,0.12)", backdropFilter: "blur(10px)", marginBottom: 14, border: "1px solid rgba(255,255,255,0.2)" }}>
                        <Ic n="shield" s={30} c="#fff" />
                    </div>
                    <h1 style={{ fontFamily: "Outfit,sans-serif", color: "#fff", fontSize: 28, fontWeight: 800, letterSpacing: "-0.5px" }}>PARIVESH 3.0</h1>
                ...
                <div style={{ background: "rgba(255,255,255,0.97)", borderRadius: 24, padding: 34, boxShadow: "0 30px 80px rgba(0,0,0,0.3)" }}>
                    ...
                    <button className="btn btn-primary" style={{ width: "100%", justifyContent: "center", padding: "11px 20px", fontSize: 15 }} onClick={go}><Ic n="shield" s={15} />Sign In Securely</button>
