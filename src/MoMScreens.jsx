...
export const MoMEd = ({ apps, upd, notify }) => {
    ...
    const pick = id => { const a = apps.find(x => x.id === id); setSel(id); setGist(a.gist || ""); setMom(a.mom || ""); setTab(a.mom ? "mom" : "gist"); };
    const savGist = () => { upd(sel, { gist }); notify("Gist saved"); };
    const toMoM = () => { const m = gist.replace("MEETING GIST", "MINUTES OF THE MEETING").replace("Date:", "MoM Date:") + "\n\n— END OF MINUTES —\nIssued by: MoM Secretariat, MoEFCC\nRef No: MOM/" + sel + "/2025"; setMom(m); upd(sel, { mom: m }); setTab("mom"); notify("MoM generated"); };
                ...
                {app ? <div className="card" style={{ padding: 22 }}>
                    ...
                    {app.locked ? <div>
                        ...
                        {tab === "gist" && <>
                            ...
                            <textarea value={gist} onChange={e => setGist(e.target.value)} rows={13} style={{ width: "100%", padding: 12, border: "1.5px solid #dce3ef", borderRadius: 9, fontFamily: "'Courier New',monospace", fontSize: 12, lineHeight: 1.7, outline: "none", resize: "vertical" }} />
                            <div style={{ display: "flex", gap: 9, marginTop: 12 }}>
                        ...
                        {tab === "mom" && <>
                            ...
                            <textarea value={mom} onChange={e => setMom(e.target.value)} rows={13} style={{ width: "100%", padding: 12, border: "1.5px solid #dce3ef", borderRadius: 9, fontFamily: "'Courier New',monospace", fontSize: 12, lineHeight: 1.7, outline: "none", resize: "vertical" }} />
                            <div style={{ display: "flex", gap: 9, marginTop: 12 }}>
